...
public:
    AreaChunkRender(uint32 flags, const std::vector<uint8>& payload, float baseX, float baseY);
    ~AreaChunkRender();

    float getMaxHeight() const { return mMaxHeight; }
    float getAverageHeight() const { return mAverageHeight; }

    bool hasHeightmap() const { return (mFlags & 1u) != 0; }
    void render();
